<h1 align="center">👋 Hi, I'm <span style="color:#00BFFF;">Shubham Kumar</span></h1>

<p align="center">
 <strong>Data Science & Cybersecurity Enthusiast</strong> | 🤖 Machine Learning | 🔐 Network Security   
<br>🎓 <em>Technical Content Writer @ Hen Harrier Education Pvt Ltd</em>
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/shubhamkr0021/CBT-CIP/refs/heads/main/giphy.gif"/>
</p>

---

## 🧑‍💻 About Me

https://tryhackme-badges.s3.amazonaws.com/shubhamkr.png?v=2



I’m passionate about data, security, and AI — building intelligent solutions that make systems smarter and safer.
Right now, I’m at Hen Harrier Education Pvt Ltd, working on cool projects in machine learning and cybersecurity.
Before this, I interned at Gilbert Research Centre, exploring AI for medical imaging and virtual reality.

💡 My focus lies in bridging the gap between **data-driven intelligence** and **secure computing systems**, merging analytical thinking with ethical hacking.

---

## Tech Stack

**🧠 Data Science:**  
`Python` • `Scikit-learn` • `TensorFlow` • `Pandas` • `NumPy` • `Matplotlib` • `Seaborn` • `Plotly` • `SQL`

**🔐 Cybersecurity:**  
`Kali Linux` • `Burp Suite` • `Nmap` • `Metasploit` • `Nessus` • `Wireshark` • `BloodHound`

**⚙️ Tools & IDEs:**  
`Google Colab` • `Jupyter Notebook` • `VS Code` • `Virtual Box` 

---

## 🧾 Professional Experience

**🖋️ Hen Harrier Education Pvt Ltd** — *Technical Content Writer & Project Specialist (Apr 2025 – Present)*  
- Authored and implemented **ML & Cybersecurity projects** for global university students.  
- Produced technical reports involving **AI model development and penetration testing**. 

**🧑‍🔬 Gilbert Research Centre** — *Research Intern (Dec 2024 – Apr 2025)*  
- Contributed to **medical imaging and AI research** projects.  
- Developed early prototypes for **VR-based diagnostic visualization**.  

**💻 EH1 Infotech, Mohali** — *Cybersecurity Intern*  
- Conducted **vulnerability assessments** using Nessus, Nmap, and Metasploit.  
- Applied **OWASP Top 10** to assess and mitigate web application risks.

---

## Featured Projects

### Data Science Projects

|  Project |  Description | 🔗 Link |
|------------|----------------|---------|
| 🩺 **Diabetes Prediction** | Predicts diabetes using Logistic Regression, Decision Tree, and SVM with 85% accuracy. | [View Repo](https://github.com/shubhamkr0021/Diabetes_Prediction) |
| 💳 **Credit Card Fraud Detection** | Uses SMOTE + Random Forest to identify fraud with class imbalance handling. | [View Repo](https://github.com/shubhamkr0021/CreditCard_FraudDetection) |
| 🎬 **IMDB Sentiment Analysis** | NLP model to classify IMDB reviews (Positive/Negative). | [View Repo](https://github.com/shubhamkr0021/DataScience_Projects/blob/main/IMDB_Sentiments_Analysis/IMDB_Sentiments_Analysis.ipynb) |
| ✈️ **Aerojet Visualization** | Exploratory visualization of aerospace performance data. | [View Repo](https://github.com/shubhamkr0021/DataScience_Projects/blob/main/Aerojet_Visualisation/Aerojet_Visualisation.ipynb) |

---

### 🔐 Cybersecurity Projects

| 🔒 Project | ⚙️ Description | 🔗 Link |
|------------|----------------|---------|
| 🧱 **P.T on Windows & Linux Systems** | Configured AD, RBAC, permissions, and system hardening using Nessus & PingCastle. | [View Repo](https://github.com/shubhamkr0021/Cybersecurity_Projects/tree/main/Ethical%20Hacking) |
| 🩸 **Active Directory Exploitation (BloodHound)** | Mapped AD privileges and achieved Domain Admin via GenericAll permission abuse. | [View Repo](https://github.com/shubhamkr0021/Cybersecurity_Projects/tree/main/BloodHound) |
| 💣 **Black Box Penetration Testing** | Simulated attacks on enterprise network discovering 10 hidden flags. | [View Repo](https://github.com/shubhamkr0021/Cybersecurity_Projects/tree/main/BlackBox_PT) |

---

## Key Achievements
- 💳 Built **fraud detection systems** achieving 99.7% AUC with Random Forest.  
- 🔍 Authored over **50+ technical reports** integrating ML and cybersecurity methods.  
- 🧩 Contributed to **penetration testing frameworks** replicating real-world attack chains.  
- 🏅 Active learner on **TryHackMe** and **Coursera Data Science Specialization**.

---

## 📜 Certifications
- 🎖️ **IBM Cybersecurity Analyst – Coursera**  
- 🎓 **DATA SCIENCE MASTERS – PwSkills**  
- 🧩 **Data Science & Machine Learning – Gilbert Research Centre**
-   **Cloud computing with AWS** - Internshala trainings**

---

## 🌐 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/shubham-kumar-61ab36135"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"></a>
  <a href="mailto:iams.k@outlook.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"></a>
  <a href="https://tryhackme.com/p/shubhamkr"><img src="https://img.shields.io/badge/TryHackMe-000000?style=for-the-badge&logo=tryhackme&logoColor=red"></a>
  <a href="https://github.com/shubhamkr0021"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"></a>
</p>

---

<h3 align="center">⚡ “Transforming data into intelligence and security into resilience.” ⚡</h3>
<p align="center">🌱 Always learning, innovating, and ready for new challenges 🚀</p>
